    ...
    private void loadWXfromLocal(boolean reload) {
        ...
        mContainer.post(new Runnable() {
            ...
            public void run() {
                ...
                if (com.taobao.weex.common.Constants.Scheme.LOCAL.equals(mUri.getScheme())) {
                    String path = mUri.getPath().substring(1);
                    mInstance.render(TAG, WXFileUtils.loadAsset(path, WXPageActivity.this),
                            mConfigMap, null,
                            WXRenderStrategy.APPEND_ASYNC);
                } else {
                    WXLogUtils.e("unsupport " + mUri.getScheme() + " protocol.");
                }
            }
        });
    }
